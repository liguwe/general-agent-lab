...
本仓库不是泛泛的 Agent 生态综述，也不追逐所有工具。当前源码层面只研究 Codex CLI 和 opencode：Codex CLI 是第一阶段主读样本，opencode 是开放产品型对照，用来帮助看清同一类问题的不同实现方式。除 `codex/` 和 `opencode/` 外，其他工具源码当下不进入研究范围。
