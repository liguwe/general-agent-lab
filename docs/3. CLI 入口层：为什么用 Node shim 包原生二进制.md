...
- Codex 和 opencode 用 Node shim，不是因为核心要用 JS 写，而是因为 npm 很适合做开发者 CLI 的分发入口。
- npm 负责安装、命令暴露、多平台二进制选择和更新，真正运行的还是 Rust 或 Bun compile 后的原生二进制。
- Node shim 夹在中间，做平台识别、二进制定位、信号转发和退出码透传。
- 对 `mini-code-agent-cli` 来说，早期可以先用纯 Node 跑通交互，等分发和性能需求变清楚后，再决定要不要拆出原生层。

## 一图甚千言

![CLI 入口层 Node shim 流程图](assets/cli-node-shim-native-binary-flow.png)

![CLI 入口层 Node shim 信息图](assets/cli-node-shim-native-binary-infographic.png)
...
核心原因是：**npm 不是运行时选择，而是分发渠道选择**。真正执行 Agent 逻辑的是 Rust 或 Bun compile 后的原生二进制，npm 负责的是安装、平台选择、命令暴露和更新。

具体拆开看：

- 一个稳定命令名：`npm i -g @openai/codex` 后，npm 会把 `codex` 挂到用户的 `PATH`；`opencode-ai` 也是同理。
- 多平台二进制分发：主包只需要放 Node shim，平台二进制可以放在 optional dependencies 里，由 npm 按当前系统拉取。
- 更新链路简单：用户继续使用 `npm update -g` 或 `npm i -g <package>@latest`，项目不用为 npm 用户单独维护一套 updater。
- 开发者生态兼容：CLI、IDE 插件、脚本、CI、本地 JS 工具链都更容易复用 npm 安装模型。
- 不牺牲原生执行：npm 只管入口和分发，真正的交互、权限、工具调用、TUI 和模型链路仍然跑在原生二进制里。
...
所以 Node shim 的价值不是“用 JS 写 CLI”，而是把 npm 这个跨平台分发层和原生二进制这个执行层接起来。换句话说：**npm 解决分发问题，原生二进制解决运行问题**。
