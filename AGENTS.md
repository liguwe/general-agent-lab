...
当前源码层面只研究 Codex CLI 和 opencode：先把这两个源码样本读清楚，抽出 Code Agent CLI 的核心骨架；每天做一个最小可运行原型，成熟后再把可复用模式沉淀进 `mini-code-agent-cli` 汇总骨架。最终要回答的是：怎么做 `General Agent Product`。

不要一上来做大而全的竞品分析，也不要把所有 AI 编程工具都纳入主线。这个仓库的重点是 Codex / opencode 源码阅读、架构拆解、双向对照表、短 notes 和每日最小可运行原型。

## 研究边界

- `codex/`：OpenAI Codex 源码（Rust），本仓库第一阶段主读对象，代表闭源产品的 CLI 实现。
- `opencode/`：anomalyco/opencode 源码（TypeScript/Bun），用来对照另一种开放产品型实现。
- 其他 Code Agent CLI 源码：当下不研究，不做源码阅读、架构拆解或对照表。
...
- 不要把 `codex/` 和 `opencode/` 的源码内容提交到本仓库，只提交 submodule 指针。
- 不要随手修改上游源码；除非任务明确要求做实验 patch。
- 搜索源码时可以直接指定目录，例如：

```bash
rg "Agent" codex opencode
rg "approval" codex opencode
rg "tool" codex opencode
```
...
每篇文章开头都必须先有 `## 一句话总结`，再有 `## 一图甚千言`，都放在标题后、正文前。`## 一句话总结` 用 3-5 条无序列表，不写连续段落；每条都是一句人话，直接说核心判断和这篇文章到底讲明白了什么。不要在这里堆太多细节、源码证据或无聊的铺垫。

`## 一图甚千言` 放一张真实图片，不用 Mermaid 或 SVG 顶替。图片按 `gen-img` 的规划做：先读文章主线，提炼核心结构，再调用 GPT 生图模型生成中文信息图 / 流程图 / 架构图，保存到合适的本地资源目录，并在这一节里嵌入。

文章整体优先使用 `outline` 大纲笔记结构：用 `##` 到 `######` 做左侧大纲锚点，用无序列表承载局部判断和细节；只有真实步骤、顺序或排名时才用有序列表。不要写大段说明文，能列要点就列要点；如果 bullet 层级变深，就把重要层级提升成标题。
...
对每个主题，尽量同时给出 Codex 和 opencode 的双向对照。不要只写结论，要标出关键文件路径和函数名。

## 协作原则

- 小步推进：一次只拆一个主题，不要同时展开太多方向。
- 证据优先：先读真实源码、命令输出或产品行为，再总结。
- 保持边界：当前源码入口只有 Codex CLI 和 opencode，其他工具只在具体问题上作为产品经验旁证，不做源码研究。
